...
INFO Spin-echo EPI (product); A>>P:Image                          | MATCH        | 19:ep2d_se_ap                       |   1.00 | True
...
INFO  -> to data series 16:ep2d_se_ap...
DEBUG     SequenceVariant: ['SK', 'SP'] != ['SK', 'SP', 'OSP']
DEBUG     NumberOfPhaseEncodingSteps: 63 != 77
DEBUG     EchoTrainLength: 31 != 39
DEBUG             91.89% match
```

```sh
pdftotext data/Template/protocol.pdf - -f 17 -l 17 -layout | \
    grep "Phase oversampling"
```
...
```sh
pdftotext data/106/protocol.pdf - -f 17 -l 17 -layout | \
    grep "Phase oversampling"
```
