...
```sh
ls data/Template/27_cmrr_mbep2d_diff_TRACEW/dicom_series/ | wc -l
27
```

```sh
ls data/010/27_cmrr_mbep2d_diff_TRACEW/dicom_series/ | wc -l
108
```
...
```sh
$ pdftotext data/Template/protocol.pdf - -f 34 -l 34 -layout | grep "Diffusion mode" | uniq | sed 's/  */ /g' | cut -d' ' -f3
MDDW
```

```sh
$ pdftotext data/010/protocol.pdf - -f 34 -l 34 -layout | grep "Diffusion mode" | uniq | sed 's/  */ /g' | cut -d' ' -f3
FREE
```
