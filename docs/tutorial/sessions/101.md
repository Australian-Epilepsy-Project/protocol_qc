...
INFO Dual echo gradient echo field map:PhaseDiff                  | PARTIAL      | 2:gre_field_mapping                 |   0.97 | True
...
INFO Dual echo gradient echo field map        | NOMATCH                              | 0.00   | True
...
```text
...
INFO  Comparing series template: Dual echo gradient echo field map:PhaseDiff
...
INFO  -> to data series 2:gre_field_mapping...
DEBUG     SequenceVariant: SP != SS
DEBUG             97.14% match
...
```

```sh
pdftotext data/Template/protocol.pdf - -f 6 -l 6 -layout | \
    grep "RF spoiling"
```
...
```sh
pdftotext data/101/protocol.pdf - -f 6 -l 6 -layout | \
    grep "RF spoiling"
```
