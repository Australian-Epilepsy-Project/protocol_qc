...
INFO Resting-state fMRI BOLD:Image                                | PARTIAL      | 21:ep2d_fid_basic_bold_p2_rest      |   0.97 | True
...
INFO Resting-state fMRI BOLD                  | NOMATCH                              | 0.00   | True
...
```text
...
INFO  Comparing series template: Resting-state fMRI BOLD:Image
INFO  -> to data series 21:ep2d_fid_basic_bold_p2_rest...
DEBUG     SpacingBetweenSlices: 5 != 5.4
DEBUG             97.30% match
...
```
...
pdftotext data/Template/protocol.pdf - -f 27 -l 27 -layout | \
    grep "Dist\. factor" | \
    tail -n1
```
...
```sh
# Note: Reproducing this command call requires installation of the Xpdf package
pdftotext data/111/protocol.pdf - -f 27 -l 27 -layout | \
    grep "Dist\. factor" | \
    tail -n1
```
