...
INFO Spin-echo EPI (product); A>>P:Image                          | MATCH        | 16:ep2d_se_ap                       |   1.00 | True
...
INFO Spin-echo EPI (product); A>>P            | DUPLICATES (UNEXPECTED)              | 0.50   | True
...
```text
...
INFO  Comparing series template: Spin-echo EPI (product); A>>P:Image
INFO  -> to data series 19:ep2d_se_ap...
DEBUG     ScanOptions: FS != ['PFP', 'FS']
DEBUG     NumberOfPhaseEncodingSteps: 63 != 56
DEBUG     EchoTrainLength: 31 != 28
DEBUG             91.89% match
...
```

```sh
pdftotext data/Template/protocol.pdf - -f 23 -l 23 -layout | \
    grep "Phase partial Fourier" | \
    sed 's/  */ /g' | \
    cut -d' ' -f1-4
```
...
```sh
pdftotext data/109/protocol.pdf - -f 23 -l 23 -layout | \
    grep "Phase partial Fourier" | \
    sed 's/  */ /g' | \
    cut -d' ' -f1-4
```
