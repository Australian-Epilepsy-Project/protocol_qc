...
Similarly to session `008`,
the software reports no issue if checked against template `03_checkparams.json`,
but checking against template `04_additional.json` produces:
...
INFO Task-based fMRI BOLD:Image                                   | MATCH        | 18:ep2d_fid_basic_bold_p2_task      |   1.00 | False
...
INFO Task-based fMRI BOLD                     | MATCH                                | 1.00   | False
...
The difference between session `009` and the prior session `008` above
is that:

1.  Here it is the task fMRI acquisition that is incomplete,
    rather than the resting-state fMRI acquisition.

2.  Here it was not a *premature termination* of the sequence
    that has resulted in the data being incomplete,
    but an *incomplete data transfer*.
    This has been simulated through selective deletion
    of an individual `.dcm` file:
    ```sh
    ls data/009/18_ep2d_fid_basic_bold_p2_task/dicom_series/
    ...
    0038.dcm
    0039.dcm
    0040.dcm
