...
INFO Task-based fMRI BOLD:Image                                   | PARTIAL      | 18:ep2d_fid_basic_bold_p2_task      |   0.97 | True
...
```text
...
INFO  Comparing series template: Task-based fMRI BOLD:Image
INFO  -> to data series 18:ep2d_fid_basic_bold_p2_task...
DEBUG     ImageType: ['ORIGINAL', 'PRIMARY', 'PERFUSION', 'NONE', 'ND'] != ['ORIGINAL', 'PRIMARY', 'PERFUSION', 'NONE', 'DIS2D']
DEBUG             97.30% match
...
```

```sh
pdftotext data/Template/protocol.pdf - -f 21 -l 21 -layout | \
    grep "Distortion Corr\." | \
    head -n1 | \
    sed 's/  */ /g' | \
    rev | \
    cut -d' ' -f1-3 | \
...
```sh
pdftotext data/108/protocol.pdf - -f 21 -l 21 -layout | \
    grep "Distortion Corr\." | \
    head -n1 | \
    sed 's/  */ /g' | \
    rev | \
