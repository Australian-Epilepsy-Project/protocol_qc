...
INFO Task-based fMRI BOLD:Image                                   | NO MATCH     |                                     |      0 |
...
INFO ------------------------------------------------------------------------------------------------------------------------------------------
WARNING 18:ep2d_fid_basic_bold_p2_OTHERtask
INFO ------------------------------------------------------------------------------------------------------------------------------------------
...
INFO Task-based fMRI BOLD                     | NOMATCH                              | 0.00   | True
...
There are two closely-related discordances reported here:

1.  The task-based BOLD fMRI sequence is unmatched.
2.  There is a task-based BOLD fMRI sequence that is classified as unmatched.

What has occurred here is that the `SeriesDescription` of the task-based fMRI acquisition
has been modified to simulate the situation
where a task *other than that planned* has been performed during the imaging session.
The nature of a task performed
is typically not intrinsically encoded within any specific DICOM headers.
It will typically be conveyed only in the content of `SeriesDescription`.
So while earlier in this tutorial
it was strongly encouraged to *not* rely on the content of `SeriesDescription`
for the prospect of detection of an imaging session performing the incorrect task
it is suggested to include a suitably-crafted regular expression
testing the content of `SeriesDescription`.
In this instance,
regex "`^.*[^a-zA-Z0-9](task|Task|TASK)$`"
has been specifically crafted to *not* achieve a match
to `SeriesDescription` "`ep2d_fid_basic_bold_p2_OTHERtask`"
